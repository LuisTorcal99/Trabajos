...
namespace RestAPI.Repository
{
    public class HouseRepository : IHouseRepository
    {
        ...
        public async Task<ICollection<HouseEntity>> GetAllAsync()
        {
            ...
            var houseFromDb = await _context.Houses.OrderBy(c => c.Name).ToListAsync();
        ...
        {
            if (_cache.TryGetValue(HouseEntityCacheKey, out ICollection<HouseEntity> SovietTanksCached))
            {
                var HouseEntity = SovietTanksCached.FirstOrDefault(c => c.Id == id);
                if (HouseEntity != null)
                    return HouseEntity;
            }
